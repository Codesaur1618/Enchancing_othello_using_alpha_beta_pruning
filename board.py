...
class Board():
    ...
    def _increment_move(move, direction):
        ...
        new_move = tuple(move[i] + direction[i] for i in range(len(move)))
        while all(0 <= new_move[i] < 8 for i in range(len(new_move))):
            yield new_move
            new_move = tuple(new_move[i] + direction[i] for i in range(len(new_move)))
